...
public class Parser {
    ...
    private void intTreatment(String line){
        ...
        if(equalPosition != -1){ //Verifica se tem sinal de =
            ...
            for (String posEqual : posEquals) {  //coloca td expressao em uma unica string
                concatLine = concatLine.concat(posEqual);
            }
            int value = (int) Operation.chooseOperation(concatLine, "int", variables);
            integer = new VariableInt(preEquals[1], value);
        }
        else{
            integer = new VariableInt(preEquals[1]);
        }
        variables.put(integer.name, integer);
    }

    private void floatTreatment(String line){
        VariableFloat pfloat;
        String concatLine = "";
        int equalPosition = line.indexOf("=");
        String [] preEquals =Util.lineInWordArray(line);
        ...
        if(equalPosition != -1){ //Verifica se tem sinal de =
            ...
            pfloat = new VariableFloat(preEquals[1], value);
        }
        else{
            pfloat = new VariableFloat(preEquals[1]);
        }
        variables.put(pfloat.name, pfloat);
    }

    private void doubleTreatment(String line){
        VariableDouble pfloat;
        String concatLine = "";
        int equalPosition = line.indexOf("=");
        String [] preEquals =Util.lineInWordArray(line);
        ...
        if(equalPosition != -1){ //Verifica se tem sinal de =
            ...
            pfloat = new VariableDouble(preEquals[1], value);
        }
        else{
            pfloat = new VariableDouble(preEquals[1]);
        }
        variables.put(pfloat.name, pfloat);
    }

    private void stringTreatment(String line){
        VariableString integer;
        if(line.contains("=")){
            ...
            if(inQuotes.contains("+")){
                ...
                integer = new VariableString(varName[1], STRcomplete);
            }
            else{
                String[] splitQuotes = inQuotes.split("'");
                integer = new VariableString(varName[1], splitQuotes[1]);
            }

        }
        else{
            String[] varName = Util.lineInWordArray(line);
            integer = new VariableString(varName[1]);
        }
        variables.put(integer.name, integer);
    }
    ...
    private void scanTreatment(String line){
        ...
        if(variables.containsKey(variable)){
            ...
            switch (variables.get(variable).type) {
                case "int" -> variables.get(variable).setValue(Integer.parseInt(lineScan));
                case "float" -> variables.get(variable).setValue(Float.parseFloat(lineScan));
                case "double" -> variables.get(variable).setValue(Double.parseDouble(lineScan));
                default -> variables.get(variable).setValue(lineScan);
            }
        }
        else{
    ...
    private int ifTreatment(String[] lines, int posLine){
        ...
        while(countIf > 0){
            ...
            if(lines[posLine].contains("else") && countIf == 1){
                findElse = true;
            }
            if(lines[posLine].contains("endif")){
                countIf--;
            }
            else if(lines[posLine].contains("if")){
                countIf++;
            }
        }
    ...
    private int forTreatment(String[] lines, int posLine){
        ...
        while(countFor > 0){
            ...
            if(lines[posLine].contains("endfor")){
                countFor--;
            }
            else if(lines[posLine].contains("for")){
                countFor++;
            }
        }
        parseInsideFor = Util.removeArray(parseInsideFor.length, parseInsideFor, parseInsideFor.length-1);

        if(forController[0].contains("int")){
            intTreatment(forController[0]);
        }
        else if(forController[0].contains("=")){
    ...
    private int whileTreatment(String[] lines, int posLine){
        ...
        while(countWhile > 0){
            ...
            if(lines[posLine].contains("endwhile")){
                countWhile--;
            }
            else if(lines[posLine].contains("while")){
                countWhile++;
            }
        }
